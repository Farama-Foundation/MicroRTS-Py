...
class MicroRTSGridModeVecEnv:
    ...
    def __init__(self,
        ...
        self.single_observation_space = gym.spaces.Box(low=0.0,
            high=1.0,
            shape=(self.height, self.width,
                    sum(self.num_planes)),
                    dtype=np.int32)

        self.single_action_space = gym.spaces.MultiDiscrete(np.array([[6, 4, 4, 4, 4, len(self.utt['unitTypes']), 7 * 7]] * self.height * self.width).flatten())
...
class MicroRTSBotVecEnv(MicroRTSGridModeVecEnv):
    ...
    def __init__(self,
        ...
        if partial_obs:
            self.num_planes = [5, 5, 3, len(self.utt['unitTypes'])+1, 6, 2]
        self.single_observation_space = gym.spaces.Discrete(2)
