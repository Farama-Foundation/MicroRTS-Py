"""
this file does action masking by subtraction the invalid logits, creating the masked categorical distribution
and then use the categorical distribution "after the subtraction" to generate the log probabilities
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gym-id', type=str, default="MicrortsGlobalAgentAttack10x10FrameSkip9-v0",
...
class CategoricalAdjust(Categorical):

    def __init__(self, probs=None, logits=None, validate_args=None, adjustment=None):
        super(CategoricalAdjust, self).__init__(probs, logits+adjustment, validate_args)
...
while global_step < args.total_timesteps:
    ...
    for step in range(args.episode_length):
        ...
        expert_action = expert_replay[step]
        ...
        if isinstance(env.action_space, Discrete):
            ...
            scale = 100
            i=0
            temp = torch.BoolTensor(1-env.unit_location_mask).to(device)
            temp2 = torch.where(temp, -logits_categories[i]+torch.tensor(-1e+8).to(device), torch.tensor(0.).to(device))
            temp2[0,expert_action[i]] += scale*logits_categories[i][0,expert_action[i]].abs().detach()
            # raise
            probs_categories.append(CategoricalAdjust(logits=logits_categories[i], adjustment=temp2))
            ...
            for i in range(1, len(logits_categories)-1):
                temp2 = torch.zeros_like(logits_categories[i])
                temp2[0,expert_action[i]] += scale*logits_categories[i][0,expert_action[i]].abs().detach()
                probs_categories.append(CategoricalAdjust(logits=logits_categories[i], adjustment=temp2))
                if len(action) != env.action_space.shape:
                    action.append(probs_categories[i].sample())
            ...
            temp = torch.BoolTensor(1-env.target_unit_location_mask).to(device)
            temp2 = torch.where(temp, -logits_categories[i]+torch.tensor(-1e+8).to(device), torch.tensor(0.).to(device))
            temp2[0,expert_action[i]] += scale*logits_categories[i][0,expert_action[i]].abs().detach()
            # raise
            probs_categories.append(CategoricalAdjust(logits=logits_categories[i], adjustment=temp2))
        ...
        dones[step] = step >= len(expert_replay)-1
    ...
    print(list(pg.fc.children())[0].weight.grad.sum())
    print(logits_categories[i][0,expert_action[i]])
    optimizer.step()
