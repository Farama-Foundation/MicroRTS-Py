...
class QNetwork(nn.Module):
    def __init__(self):
        ...
        self.network = nn.Sequential(
            layer_init(nn.Conv2d(27, 16, kernel_size=2)),
            nn.ReLU(),
            layer_init(nn.Conv2d(16, 32, kernel_size=1)),
            nn.ReLU(),
            nn.Flatten(),
...
for global_step in range(args.total_timesteps):
    ...
    if global_step > args.learning_starts and global_step % args.train_frequency == 0:
        ...
        with torch.no_grad():
            ...
            target_max = torch.stack([l.max(1)[0] for l in valid_logits]).sum(0) / len(env.action_space.nvec.tolist())
            td_target = torch.Tensor(s_rewards).to(device) + args.gamma * target_max * (1 - torch.Tensor(s_dones).to(device))
        _, old_valid_logits = q_network.get_action(s_obs)
        old_val = torch.cat([l.gather(1, a.view(-1,1)) for (a, l) in 
             zip(torch.LongTensor(s_actions.T).to(device), old_valid_logits)],1).sum(1) / len(env.action_space.nvec.tolist())
        loss = loss_fn(td_target, old_val)

        if global_step % 100 == 0:
            writer.add_scalar("losses/td_loss", loss, global_step)
            print(valid_logits[0][0].max())
