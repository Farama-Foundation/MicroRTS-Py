...
from gym_microrts.envs.gym_vec_env import MicroRTSGridModeVecEnv
import numpy as np
import torch
...
class Agent(nn.Module):
    def __init__(self, envs, mapsize=16*16):
        ...
        _, _, c = envs.single_observation_space.shape
        self.network = nn.Sequential(
            ...
            nn.ReLU(),)
        print(self.mapsize*envs.action_plane_space.nvec.sum())
        self.actor = layer_init(nn.Linear(256, self.mapsize*envs.action_plane_space.nvec.sum()), std=0.01)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        return action, logprob.sum(1).sum(1), entropy.sum(1).sum(1), self.critic(hidden)
