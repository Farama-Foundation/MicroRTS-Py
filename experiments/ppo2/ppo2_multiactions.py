...
if __name__ == "__main__":
    ...
    parser.add_argument('--gym-id', type=str, default="MicrortsCombinedReward10x10F5MultiActionsBuildCombatUnits-v0",
...
class Policy(nn.Module):
    ...
    def get_action(self, x, multiple_actions=None, invalid_action_masks=None, num_actions=2):
        lobprobs = []
        ...
        if multiple_actions is None:
            ...
            for _ in range(num_actions):
                ...
                multiple_actions += [action]
                lobprobs += [logprob]
                
        else:
            lobprobs = torch.zeros(len(x))
            for idx, action in enumerate(multiple_actions):
                split_logits = torch.split(logits[idx:idx+1], env.action_space.nvec.tolist(), dim=1)
                split_invalid_action_masks = torch.split(invalid_action_masks[idx:idx+1], env.action_space.nvec.tolist(), dim=1)
                multi_categoricals = [CategoricalMasked(logits=logits, masks=iam) for (logits, iam) in zip(split_logits, split_invalid_action_masks)]
                lobprobs[idx] = torch.stack([categorical.log_prob(a).sum() for a, categorical in zip(action, multi_categoricals)]).sum()
            return None, lobprobs, [], multi_categoricals
        return torch.cat(multiple_actions,1), torch.cat(lobprobs,1).sum(), [], multi_categoricals
...
while global_step < args.total_timesteps:
    ...
    actions = np.empty((args.batch_size,), dtype=object)
    ...
    for step in range(args.batch_size):
        ...
        next_obs, rewards[step], dones[step], info = env.step(action.T.tolist())
