...
class Policy(nn.Module):
    ...
    def get_action(self, x, action=None, invalid_action_masks=None):
        ...
        target_multi_categoricals = [Categorical(logits=logits) for logits in split_logits]
        
        if action is None:
            action = torch.stack([categorical.sample() for categorical in multi_categoricals])
        logprob = torch.stack([categorical.log_prob(a) for a, categorical in zip(action, target_multi_categoricals)])
        importance_sampling = torch.stack(
            [target.probs[0,a]/behavior.probs[0,a] for a, behavior, target in 
             zip(action, multi_categoricals, target_multi_categoricals)])
        behavior_logprob = torch.stack([categorical.log_prob(a) for a, categorical in zip(action, multi_categoricals)])
        return action, logprob, [], multi_categoricals, importance_sampling, behavior_logprob
...
while global_step < args.total_timesteps:
    ...
    for step in range(args.batch_size):
        ...
        with torch.no_grad():
            ...
            action, _, _, probs, iss, logproba = pg.get_action(obs[step:step+1], invalid_action_masks=invalid_action_masks[step:step+1])
    ...
    for i_epoch_pi in range(int(args.update_epochs * args.batch_size / args.minibatch_size)):
        ...
        _, newlogproba , _, _, _, _ = pg.get_action(
            obs[minibatch_ind],
