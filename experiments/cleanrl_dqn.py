...
if __name__ == "__main__":
    ...
    parser.add_argument('--gym-id', type=str, default="MicrortsGlobalAgentsMaxResources4x4Prod-v0",
    ...
    parser.add_argument('--exploration-duration', type=int, default=250000 ,
...
while global_step < args.total_timesteps:
    ...
    for step in range(args.episode_length):
        ...
        target = torch.split(target_network.forward(s_next_obses), env.action_space.nvec.tolist(), dim=1)
        old_val = torch.split(q_network.forward(s_obs), env.action_space.nvec.tolist(), dim=1)
        target_q = torch.zeros((args.batch_size))
        old_val_q = torch.zeros((args.batch_size)) 
        for i in range(len(target)):
            target_q += target[i].max(dim=1)[0]
            old_val_q += old_val[i].gather(1, torch.LongTensor(s_actions[:,0]).view(-1,1)).squeeze()
        
        td_target = torch.Tensor(s_rewards) + args.gamma * target_q * (1 - torch.Tensor(s_dones))
        loss = loss_fn(td_target, old_val_q)
