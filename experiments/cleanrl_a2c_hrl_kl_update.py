...
while global_step < args.total_timesteps:
    ...
    unit_location_masks = np.empty((args.episode_length,), dtype=object)
    target_unit_location_masks = np.empty((args.episode_length,), dtype=object)
    ...
    for step in range(args.episode_length):
        ...
        unit_location_masks[step] = env.unit_location_mask.copy()
        target_unit_location_masks[step] = env.target_unit_location_mask.copy()
    ...
    for _ in range(10):
        ...
        vf_loss = ((torch.Tensor(returns[:,random_idx]).to(device) - values[:,random_idx])**2).mean(1) * args.vf_coef
        pg_loss = torch.Tensor(advantages[:,random_idx]).to(device) * neglogprobs[:,random_idx]
        entropy_loss = (-entropys[:,random_idx]) * args.ent_coef
        ...
        ######
        # KL divergence ugly hack
        ######
        new_neglogprobs = torch.zeros((num, args.episode_length,), device=device)
        for s in range(step+1):
            logitss = [pg.forward(obs[s:s+1]) for pg in pgs]
            all_logits_categories =  []
            action = []
            all_probs_categories = []
            all_probs_entropies =  []
            ...
            for i in range(num):
                j=0
                all_probs_categories[i].append(CategoricalMasked(logits=all_logits_categories[i][j], masks=unit_location_masks[s]))
            for i in range(num):
                for j in range(1, len(all_logits_categories[0])-1):
                    all_probs_categories[i].append(Categorical(logits=all_logits_categories[i][j]))
            for i in range(num):
                j=len(all_logits_categories[0])-1
                all_probs_categories[i].append(CategoricalMasked(logits=all_logits_categories[i][j], masks=target_unit_location_masks[s]))
    
            
            for i in range(num):
                for j in range(0, len(all_logits_categories[0])):
                    all_neglogprob[i] -= all_probs_categories[i][j].log_prob(torch.tensor(actions[s][j]).to(device))
                    all_probs_entropies[i] += all_probs_categories[i][j].entropy()
