...
def parse_args():
    ...
    # fmt: on
    return wrap_args(args)


def wrap_args(args):
    if hasattr(args, 'is_wrapped'):
        return args
...
# xxx(okachaiev): add support for PvE evaluation
def evaluate(args, agent_path, opponent_path):
    # xxx(okachaiev): i need to find a better way of dealing with the API like this
    args = wrap_args(args)
    wins, losses, draws = 0, 0, 0
    device = torch.device("cuda" if torch.cuda.is_available() and args.cuda else "cpu")

    # xxx(okachaiev): would be nice if we can setup this with initializer and clean up later
    ...
    agent2.load_state_dict(torch.load(opponent_path, map_location=device))
    agent2.eval()
    ...
    for _ in range(args.total_timesteps):
        ...
        with torch.no_grad():
            mask = torch.tensor(np.array(envs.get_action_mask())).to(device)
            ...
            action = torch.zeros((envs.num_envs, p2_action.shape[1], p2_action.shape[2]))
            action[::2] = p1_action
        ...
        if ds[0]:
            p1_win_loss = infos[0]['raw_rewards'][0]
            if p1_win_loss == 1.:
                wins += 1
            elif p1_win_loss == -1.:
                losses += 1
...
# xxx(okachaiev): use second agent if provided
def train(args, agent_path=None, opponent_path=None):
    args = wrap_args(args)
    ...
    if args.num_bot_envs > 0:
        ai_bots = [microrts_ai.coacAI for _ in range(args.num_bot_envs - 6)] \
    ...
    envs = MicroRTSGridModeVecEnv(
        # xxx(okachaiev): i'm still not sure if it's possible to execute both selfplay and
        # PvE within the same env setup. seems like the approach with SlimeVolley was quite
        # decent
    ...
    for update in range(starting_update, args.num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                # )
                values[step][::2] = p1_vs.flatten()
                values[step][1::2] = p2_vs.flatten()

            actions[step] = action
            logprobs[step][::2] = p1_logproba
            logprobs[step][1::2] = p2_logproba
            try:
                next_obs, rs, ds, infos = envs.step(action.cpu().numpy().reshape(envs.num_envs, -1))
                next_obs = torch.Tensor(next_obs).to(device)
            except Exception as e:
            ...
            # xxx(okachaiev): this only works for a single opponent
            if ds[0]:
                p1_win_loss = infos[0]['raw_rewards'][0]
                if p1_win_loss == 1.:
                    wins += 1
...
if __name__ == "__main__":
    args = parse_args()
    train(args, opponent_path="league_v2/eval/agent_sota.pt")