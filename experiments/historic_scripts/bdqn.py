...
if __name__ == "__main__":
    ...
    parser.add_argument("--gym-id", type=str, default="MicrortsMining10x10F9-v0", help="the id of the gym environment")
...
class QNetwork(nn.Module):
    ...
    def get_action(self, x, action=None, invalid_action_masks=None):
        ...
        if invalid_action_masks is not None:
            ...
            multi_categoricals = [
                torch.where(iam, logits, torch.tensor(-1e8).to(device)) for (logits, iam) in zip(split_logits, iams)
            ]
        else:
...
for global_step in range(args.total_timesteps):
    ...
    if global_step > args.learning_starts and global_step % args.train_frequency == 0:
        ...
        with torch.no_grad():
            _, valid_logits = target_network.get_action(
                s_next_obses, invalid_action_masks=torch.Tensor(s_invalid_action_masks).to(device)
            )
            target_max = torch.stack([l.max(1)[0] for l in valid_logits])
            td_target = torch.Tensor(s_rewards).to(device) + args.gamma * target_max * (1 - torch.Tensor(s_dones).to(device))
        _, old_valid_logits = q_network.get_action(s_obs)
        old_val = torch.cat(
            [l.gather(1, a.view(-1, 1)) for (a, l) in zip(torch.LongTensor(s_actions.T).to(device), old_valid_logits)], 1
        ).T
