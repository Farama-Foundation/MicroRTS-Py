...
class Agent(nn.Module):
    def __init__(self, frames=4):
        ...
        self.actor_others_head = layer_init(
            nn.Linear(256 + envs.action_space.nvec[0], envs.action_space.nvec[1:].sum()), std=0.01
        )
    ...
    def get_action(self, x, action=None, invalid_action_masks=None, envs=None):
        ...
        if action is None:
            ...
            multi_categoricals = multi_categoricals + [
                CategoricalMasked(logits=logits, masks=iam) for (logits, iam) in zip(split_logits, split_suam)
            ]
            ...
            multi_categoricals = multi_categoricals + [
                ...
                for (logits, iam) in zip(split_logits, split_invalid_action_masks[1:])
            ]
