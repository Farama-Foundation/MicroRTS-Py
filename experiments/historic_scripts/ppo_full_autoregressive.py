...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gym-id", type=str, default="MicrortsDefeatWorkerRushEnemyShaped-v3", help="the id of the gym environment"
    )
...
class Agent(nn.Module):
    ...
    def get_action(self, x, action=None, invalid_action_masks=None, envs=None):
        ...
        if action is None:
            ...
            auto_regressive_embeddings = self.actor_source_unit_embedding(
                torch.cat((hidden_output, F.one_hot(action_components[0], envs.action_space.nvec[0])), 1)
            )
            ...
            auto_regressive_embeddings = self.actor_type_embedding(
                torch.cat((auto_regressive_embeddings, F.one_hot(action_components[1], envs.action_space.nvec[1])), 1)
            )
            ...
            multi_categoricals = multi_categoricals + [
                CategoricalMasked(logits=logits, masks=iam) for (logits, iam) in zip(split_logits, split_suam[1:])
            ]
            ...
            # 2. select action type
            auto_regressive_embeddings = self.actor_source_unit_embedding(
                torch.cat((hidden_output, F.one_hot(action[0], envs.action_space.nvec[0])), 1)
            )
            at_logits = self.actor_type_head(auto_regressive_embeddings)
            multi_categoricals += [CategoricalMasked(logits=at_logits, masks=split_invalid_action_masks[1])]
            #  3. select other parameter
            auto_regressive_embeddings = self.actor_type_embedding(
                torch.cat((auto_regressive_embeddings, F.one_hot(action[1], envs.action_space.nvec[1])), 1)
            )
            other_logits = self.actor_others_head(auto_regressive_embeddings)
            split_logits = torch.split(other_logits, envs.action_space.nvec[2:].tolist(), dim=1)
            multi_categoricals = multi_categoricals + [
                CategoricalMasked(logits=logits, masks=iam)
                for (logits, iam) in zip(split_logits, split_invalid_action_masks[2:])
            ]
