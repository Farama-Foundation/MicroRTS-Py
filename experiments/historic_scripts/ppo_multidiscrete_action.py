...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gym-id", type=str, default="MicrortsCombinedReward10x10F9BuildCombatUnits-v0", help="the id of the gym environment"
    )
...
for update in range(1, num_updates + 1):
    ...
    for i_epoch_pi in range(args.update_epochs):
        ...
        if args.kle_rollback:
            if (
                b_logprobs[minibatch_ind] - agent.get_action(b_obs[minibatch_ind], b_actions.long()[minibatch_ind].T)[1]
            ).mean() > args.target_kl:
