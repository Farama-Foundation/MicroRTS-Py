...
if __name__ == "__main__":
    ...
    parser.add_argument("--gym-id", type=str, default="MicrortsSelfPlayShapedReward-v1", help="the id of the gym environment")
...
class Agent(nn.Module):
    ...
    def get_action_with_device(self, x, action=None, invalid_action_masks=None):
        x = torch.Tensor(x).to(device)
        invalid_action_masks = torch.Tensor(invalid_action_masks).to(device)
        return self.get_action(x, action, invalid_action_masks)
...
for update in range(1, num_updates + 1):
    ...
    for step in range(0, args.num_steps):
        ...
        # Self-play LOGIC:
        with torch.no_grad():
            o_action, _, _ = opponent_agent.get_action(
